...
export function CursorRevealOverlay({
  children,
  underLayer,
}: {
  ...
  const isElementOrParentLink = useCallback((element: Element): boolean => {
    ...
    while (current && current !== containerRef.current) {
      if (
        ...
        current.classList.contains('cursor-no-reveal')
      ) {
        // Optional: add custom class for exclusion
        return true;
      }
  ...
  const updateCursorTrail = useCallback(
    (x: number, y: number) => {
      ...
      setCursorTrail((prev) => {
        ...
        return newTrail
          .filter((pos) => timestamp - pos.timestamp < trailDecay)
          .slice(0, trailLength);
      });
  ...
  useEffect(() => {
    function handleMouseMove(e: MouseEvent) {
      if (containerRef.current) {
        ...
        const overLink = elementUnderCursor
          ? isElementOrParentLink(elementUnderCursor)
          : false;
  ...
  useEffect(() => {
    const cleanUpTrail = () => {
      const now = Date.now();
      setCursorTrail((prev) =>
        prev.filter((pos) => now - pos.timestamp < trailDecay)
      );
  ...
  return (
    <div ref={containerRef} className='relative w-full h-full overflow-hidden'>
      ...
      <NetworkArchitectureBackground
        cursorPosition={isOverLink ? undefined : latestCursor ?? undefined}
      />
            ...
            <div
              ...
              style={{
                ...
                maskImage: `radial-gradient(circle ${radius}px at ${
                  position.x
                }px ${position.y}px, black 0%, black ${
                  radius * 0.8
                }px, transparent ${radius + blurRadius}px)`,
