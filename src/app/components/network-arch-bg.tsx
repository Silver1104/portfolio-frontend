...
export function NetworkArchitectureBackground({
  ...
  const initializeNodes = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < numNodes; i++) {
      ...
      nodesRef.current.push({
        x: Math.max(
          NODE_TYPES[type].size,
          Math.min(width - NODE_TYPES[type].size, baseX + randomOffsetX)
        ),
        y: Math.max(
          NODE_TYPES[type].size,
          Math.min(height - NODE_TYPES[type].size, baseY + randomOffsetY)
        ),
        vx,
        vy,
        originalVx: vx,
    ...
    nodesRef.current.forEach((node, i) => {
      ...
      nodesRef.current.forEach((otherNode, j) => {
        if (
          i !== j &&
          node.connections.size < maxConnections &&
          otherNode.connections.size < maxConnections
        ) {
  ...
  const checkAndFillEmptyAreas = useCallback(
    (width: number, height: number, currentTime: number) => {
      ...
      for (let x = 0; x < width; x += gridSize) {
        for (let y = 0; y < height; y += gridSize) {
          const nodesInArea = nodes.filter(
            (node) =>
              node.x >= x &&
              node.x < x + gridSize &&
              node.y >= y &&
          ...
          if (
            nodesInArea < minNodesPerArea &&
            Math.random() < 0.1 &&
            nodes.length < maxNodesRef.current
          ) {
            const nodeTypes = Object.keys(NODE_TYPES) as Array<
              keyof typeof NODE_TYPES
            >;
            const randomTypeIndex = Math.floor(
              Math.random() * nodeTypes.length
            );
            const type = nodeTypes[randomTypeIndex] ?? 'api';
            const vx = (Math.random() - 0.5) * 0.2;
            const vy = (Math.random() - 0.5) * 0.2;
            ...
            nodes.forEach((otherNode, j) => {
              if (
                newNode.connections.size < 4 &&
                otherNode.connections.size < 4
              ) {
  ...
  const createDataPacket = useCallback(() => {
    ...
    if (fromNode && fromNode.connections.size > 0) {
      ...
      const toIndex =
        connectionsArray[Math.floor(Math.random() * connectionsArray.length)];
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.strokeStyle = `rgba(64, 224, 208, ${
        0.3 + Math.sin(currentTime * 0.002) * 0.2
      })`;
      ctx.lineWidth = 1;
      ctx.setLineDash([5, 3]);
